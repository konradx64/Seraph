...
pub async fn get_tunnels(
    ...
    let tunnels = state.db.load_tunnels()
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to load tunnels: {:?}", e)))?;

    let active = state.active_tunnels.read().await;

    let items = tunnels.into_iter().map(|t| {
        ...
        let (bytes_sent, bytes_received) = {
            ...
            if let Some(tstats) = guard.get(&t.id) {
                (
                    ...
                    tstats.bytes_received.load(std::sync::atomic::Ordering::Relaxed),
                )
            } else {
                (0, 0)
            }
...
pub async fn create_tunnel(
    ...
    if id.is_empty() {
        return Err((StatusCode::BAD_REQUEST, "Tunnel ID cannot be empty".to_string()));
    }
    ...
    state.db.save_tunnel(id, &token, &created_at)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to save tunnel: {:?}", e)))?;
...
pub async fn delete_tunnel(
    ...
    let deleted = state.db.delete_tunnel(&params.id)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to delete tunnel: {:?}", e)))?;
...
pub async fn enroll_tunnel(
    ...
    let db_tunnel = state.db.get_tunnel_by_token(&payload.token)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("DB error: {:?}", e)))?
        .ok_or_else(|| (StatusCode::UNAUTHORIZED, "Invalid enrollment token".to_string()))?;
    ...
    let mut params = rcgen::CertificateParams::new(vec![])
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Params build failed: {:?}", e)))?;
    
    params.distinguished_name.push(rcgen::DnType::CommonName, &db_tunnel.id);
    params.distinguished_name.push(rcgen::DnType::OrganizationName, "Seraph Agent");
    ...
    params.not_after = time::OffsetDateTime::now_utc()
        ...
        .ok_or_else(|| (StatusCode::INTERNAL_SERVER_ERROR, "Date overflow".to_string()))?;

    // Sign the client public key with our CA cert and key
    let cert = params.signed_by(&csr_params.public_key, &state.ca.cert, &state.ca.key_pair)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Certificate signing failed: {:?}", e)))?;

    let cert_pem = cert.pem();

    // 4. Save the issued certificate to the database
    state.db.save_tunnel_cert(&db_tunnel.id, &cert_pem)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to save cert: {:?}", e)))?;
