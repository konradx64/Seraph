...
async fn main() -> Result<()> {
    ...
    if !key_path.exists() || !cert_path.exists() || !ca_path.exists() {
        ...
        std::fs::create_dir_all(data_dir_path).context("Failed to create agent data directory")?;
...
async fn enroll(
    ...
    params.distinguished_name.push(rcgen::DnType::CommonName, "seraph-agent-enrollment");
    ...
    let response = client
        .post(&enroll_url)
        .json(&EnrollPayload { token, csr: csr_pem })
        .send()
        .await
    ...
    let enroll_res: EnrollResponse = response.json().await.context("Failed to parse enrollment response")?;

    // Persist keys and certs to data_dir
    std::fs::write(key_path, &key_pem).context("Failed to save identity.key")?;
...
async fn handle_stream(mut send_stream: quinn::SendStream, mut recv_stream: quinn::RecvStream) -> Result<()> {
...
fn build_client_config(key_path: &Path, cert_path: &Path, ca_path: &Path) -> Result<rustls::ClientConfig> {
    ...
    for cert in rustls_pemfile::certs(&mut ca_reader) {
        root_store.add(cert?).context("Failed to add CA cert to client trust store")?;
    }
